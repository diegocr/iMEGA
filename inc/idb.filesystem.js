 ...
 * @version: 0.0.2
 * 
 * This file has been modified for use in the iMEGA Mozilla Firefox Extension
 *
 *      https://addons.mozilla.org/en-US/firefox/addon/imega/
 *
 * Bugfixes and improvements were made by Diego Casorran (dcasorran@gmail.com)
 * under the terms of the original licence and released as such accordingly.
 */
...
function FileWriter(fileEntry) {
  ...
  this.write = function(blob) {
	...
	// Set writing state.
	readyState_ = this.WRITING;

    // Set the blob we're writing on this file entry so we can recall it later.
    fileEntry_.file_.blob_ = blob;
	
	// XXX: ...
	fileEntry_.chunks = fileEntry_.chunks || [];
	fileEntry_.chunks.push(blob);
...
Entry.prototype = {
  ...
  getMetadata: function(callback) {
	...
	callback({ modificationTime: { getTime: function() 0}});
  },
  getParent: function() {
  ...
  toURL: function() {
	idb_.close();
	return 'imega:' + base64urlencode((new Date().getTime() & 0xffff) + '!'
		+ location.protocol + '//' + location.host + this.fullPath + '#' + storageType_);
...
//exports.idb = idb_;
exports.webkitRequestFileSystem =
